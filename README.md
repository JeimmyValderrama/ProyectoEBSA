...
- **Glosario:** los códigos del TC2 (ciclo, clase de servicio, tipo de medidor, tipo de lectura, tipo de factura) se traducen a texto con `utilidades/utilidades_glosario.py` (tomado de `GLOSARIO_EBSA.xlsx`), y los perfiles técnicos P0–P4 se muestran como **grupos de consumo**: Intermitente, Pequeño (< 500 kWh/mes), Mediano (500–5.000), Grande (≥ 5.000) y Sin historia suficiente.

---

## 1. Qué entrega el proyecto
...
La carpeta `Pipeline_Ebsa` queda organizada así:

```text
Pipeline_Ebsa\\
│
├── pipeline_mensual.py                 ← punto de entrada del pipeline
│
├── notebooks\\
│   ├── 01_exploracion\\
...
│   ├── 05_segmentacion_clientes\\    ← notebook 9
│   ├── 06_analisis_caidas\\          ← notebooks 10 y 11
│   ├── 07_riesgo_fuga\\              ← notebook 14
│   ├── 08_seguimiento\\              ← notebooks 12 y 13
│   └── 09_exportes\\                 ← notebook 15
│
├── scripts\\                          ← automatización y diagnósticos
├── utilidades\\                       ← módulos Python compartidos
├── web\\                              ← aplicación Streamlit (borrador inicial)
└── plantillas\\                       ← formatos de entrada manual
```
...
La aplicación web se inicia con alguno de los siguientes comandos:

```bash
streamlit run web\\app_ebsa.py
...
│
├── 00_formato_TC2/
│   └── Archivos XLSX/CSV mensuales de la empresa.
│       Entrada del paso 1. No modificar.
│
├── 00_otros_comercializadores/
│   └── Archivo XLSX con los usuarios atendidos por otros comercializadores.
│       Entrada del paso 14. No modificar.
│
...
│   └── detalle_mensual/
│       └── Detalle factura a factura por archivo,
│           utilite para auditoría.
│
├── 02_serie_reconstruida/
│   ├── copia_historicos/
│   │   └── Copias de trabajo de los archivos históricos.
│   └── control_calidad_mes_entrante.csv
│
├── 03_serie_modelado/
│   └── Serie preprocesada que sirve como entrada
│       para todos los modelos.
│
├── 04_pronostico/
│   ├── desarrollo_01_modelo_unico/
│   │   └── Primer modelo, sin segmentación.
│   │       Notebook 4. Se conserva como referencia.
│   │
│   ├── desarrollo_02_segmentado/
│   │   └── Primera versión del modelo segmentado.
│   │       Notebook 5. Se conserva como referencia.
│   │
│   └── modelo_final/
│       ├── Notebooks 6, 7 y 8.
│       │   Contienen el modelo final de pronóstico.
│       └── historial_pronosticos/
│           └── Copia del pronóstico correspondiente
│               a cada fecha de corte.
│
├── 05_segmentos_clientes/
│   └── Resultados del modelo final de agrupamiento.
│       Paso 9.
│
├── 06_estudio_caida/
│   └── Criterios finales para identificar la caída.
│       Paso 10.
│
├── 07_gestion_caida/
│   ├── historial/
│   │   └── Copia de la lista operativa correspondiente
│   │       a cada fecha de corte.
│   └── retroalimentacion/
│       └── Plantillas y resultados de las visitas.
│           Paso 13.
│
├── 08_seguimiento/
│   └── Precisión en vivo y resultados obtenidos
│       con las listas generadas.
│       Paso 12.
│
├── 09_registro_corridas/
│   └── pipeline_mensual.py
│       Registro de cada corrida, incluyendo los notebooks
│       ejecutados y un resumen de resultados.
│
├── 10_riesgo_fuga/
│   ├── Modelo, listas y seguimiento del riesgo de fuga.
│   └── historial/
│       └── Copia de la lista de riesgo correspondiente
│           a cada fecha de corte.
│
└── 11_exportes_negocio/
    ├── pronostico_6_meses/
    ├── lista_caida/
    └── riesgo_fuga/
        Archivos finales organizados por grupo de consumo
...
`python prscripts\\eparar_carpeta_datos.py` (desde Pipeline_Ebsa) crea las carpetas vacías de Datos_Ebsa` y dice qué copiar del proyecto anterior.

--# 2.2 Variables de entorno y rutas del código

`pipeline_mensual.py` calcula su propia ubicación con `Path(__file__).resolve().parent`, por lo que puede ejecutarse desde cualquier carpeta. Define estas rutas internas:
...
Además de `EBSA_DATOS` y `EBSA_MODO`, el pipeline expone `EBSA_CODIGO`, `EBSA_NOTEBOOKS` y `EBSA_UTILIDADES`. También agrega las carpetas necesarias a `PYTHONPATH`, porque cada notebook se ejecuta en un kernel separado.
...
| `cluster_id` | Segmento de negocio al que pertenece. |
...
### 3.2 Producto B — Pronóstico a 6 meses 
...
### 3.2c Exportes por grupo de consumo 
...
### 3.3 Seguimiento — precisión en vivo
...
| **Detector del borde provisional** | `Pipeline_Ebsa\utilidades\utilidades_borde.py` | Función `ultimo_periodo_consolidado(serie)` → último mes que se puede usar. | Lo llaman los notebooks 8, 9, 10 y 12 antes de entrenar, agrupar, evaluar o comparar . |
| **Compuerta de calidad** | `Pipeline_Ebsa\utilidades\utilidades_calidad.py` | Función `validar_mes_entrante(historico)` → reporte por chequeo; lanza error si el archivo del mes llegó mal. | La llama el notebook 2 antes de reconstruir. |

> **En modo aplicar los tres modelos se cargan y se usan sin reentrenar.** Un mes normal no toca ningún `.joblib`; solo la corrida en modo reentrenar los sobreescribe.
...
| — | `utilidades/utilidades_borde.py` | Módulo Python (no es notebook). Detecta el borde provisional y calcula el corte por zona (`cortes_por_zona`, `leer_cortes_por_zona`). Lo importan 3, 8, 9, 10, 12 y 14. | — | — |
| — | `utilidades/utilidades_glosario.py` | Módulo Python (no es notebook). Traduce ciclo → zona, clase de servicio, tipo de medidor/lectura/factura y perfil → grupo de consumo; calcula el valor facturado. Lo importan 11, 14, 15 y la página. | — | — |
| — | `utilidades/utilidades_versiones.py` | Módulo Python. Guarda y resuelve versiones de los modelos (`12_versiones_modelos`). Lo importan 8, 9, 10 y 14. | — | — |
| — | `scripts/simular_meses.py`, `scripts/comparar_modelos.py` | Simulación mes a mes con `--corte-max` (entrenar en un corte, aplicar mes a mes, reentrenar, comparar) y lectura del deterioro / mejora. Procedimiento completo en `COMANDOS.md` §4b. | — | — |
| — | `scripts/verificar_corrida.py` | Verificador de la corrida (solo lee): consistencia de cortes, pronóstico, listas, exportes y registro. Correr después de cada `pipeline_mensual.py`. | — | — |
| — | `scripts/estado_modelos.py` | ¿Toca reentrenar? Un veredicto por modelo (MANTENER / REVISAR / REENTRENAR) leyendo la antigüedad, el seguimiento en vivo y la deriva de criterios; imprime el comando exacto. Solo lee. | — | — |
| — | `scripts/diagnostico_fuga.py`, `scripts/diagnostico_cruce_fuga.py` | Scripts de solo lectura con los que se estudió cómo se ven en TC2 los clientes que se fueron (no forman parte de la corrida). | — | — |
| — | `utilidades/utilidades_calidad.py` | Módulo Python (no es notebook). Compuerta de calidad del archivo entrante. Lo importa el 2. | — | — |
| — | `pipeline_mensual.py` | Script que corre los notebooks en orden con kernel limpio (§7). | — | `09_registro_corridas\` |
| — | `web/app_ebsa.py` | Página web (Streamlit) que muestra las salidas (§10). | — | — |
| — | `requirements.txt`, `scripts/generar_lock_entorno.py` | Dependencias del entorno . | — | — |
| — | `scripts/preparar_carpeta_datos.py` | Crea la estructura de carpetas y dice qué copiar del proyecto anterior (§7.0). | — | — |



> **Compatibilidad de organización.** La reorganización de carpetas no cambia las rutas de `Datos_Ebsa`. Solo cambia la ubicación del código. Si un notebook o script importa `utilidades_borde`, `utilidades_calidad`, `utilidades_glosario` o `utilidades_versiones`, debe ejecutarse mediante el pipeline actualizado o con `Pipeline_Ebsa\\utilidades\\` disponible en `PYTHONPATH`.
...
**Corrida mensual:** 1 → 2 → 3 → 8 → 9 → 10 → 11 → 12 → 13 → 14 → 15, en modo *aplicar*. Es exactamente lo que hace `python pipeline_mensual.py --modo aplicar`. Los notebooks 4, 5, 6 y 7 son de desarrollo y solo se repiten si se quiere volver a elegir algoritmos o hiperparámetros.

---

## 5. Qué hay en cada carpeta de Datos Ebsa  y cómo leerlo
...
### 7.0 Verificación inicial de la estructura del código

Antes de procesar datos, ejecutar desde `Pipeline_Ebsa`:

```bash
python pipeline_mensual.py --lista
```

Todos los notebooks seleccionables deben aparecer sin `NO ENCONTRADO`. Si falta una utilidad o un notebook, revisar la estructura de `notebooks\\` y `utilidades\\` antes de ejecutar una corrida.

Para probar un paso pequeño:

```bash
python pipeline_mensual.py --modo aplicar --solo 1
```
...
5. Abrir la página (`streamlit run web\app_ebsa.py`) o leer directamente `07_gestion_caida\`, `04_pronostico\modelo_final\`, `08_seguimiento\`, `10_riesgo_fuga\` y, para repartir a negocio, `11_exportes_negocio\`.
...
python pipeline_mensual.py --modo reentrenar                # corrida de reentrenamiento
...
`web/app_ebsa.py` es una aplicación Streamlit que **solo lee** las salidas del pipeline (no entrena ni recalcula nada). Para abrirla:

```
pip install streamlit        # una sola vez
streamlit run web\app_ebsa.py    # se abre en http://localhost:8501
```
